...
/**
 * Staff sessions — the storefront has no accounts, so the admin is the only
 * place anyone signs in. A random 256-bit token in an httpOnly cookie; KV holds the record
...
const TTL: Record<Kind, number> = { staff: 60 * 60 * 12 };
const COOKIE: Record<Kind, string> = { staff: COOKIES.staffSession };
...
export async function createSession(c: Ctx, kind: Kind, sub: string, epoch: number): Promise<string> {
  ...
  writeCookie(c, COOKIE[kind], token, { maxAge: TTL[kind], sameSite: 'Strict' });
  return key;
}
...
async function maybeExtend(c: Ctx, kind: Kind, s: { record: SessionRecord; key: string; token: string }) {
  ...
  writeCookie(c, COOKIE[kind], s.token, { maxAge: TTL[kind], sameSite: 'Strict' });
}
